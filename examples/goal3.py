...
from rlkit.torch.sac.gcs.gcs import GCSHierarchicalTrainer
from rlkit.torch.networks import FlattenMlp
from rlkit.torch.sac.gcs.skill_discriminator import SkillDiscriminator
from rlkit.torch.sac.gcs.gcs_torch_rl_algorithm import GCSTorchRLAlgorithm2
...
def experiment(variant, args):
    ...
    sd_replay_buffer = GCSEnvReplayBuffer(
        variant['sd_buffer_size'],
        expl_env,
        skill_dim,
        ends_dim,
    )
    goal_path_buffer = GCSSGoalPathBuffer(
        variant['goal_buffer_size'],
        obs_dim,
        ends_dim,
        skill_dim,
    )
    trainer = GCSHierarchicalTrainer(
        env=eval_env,
        policy=policy,
        qf1=qf1,
        qf2=qf2,
        skill_discriminator=skill_discriminator,
    ...
    algorithm = GCSTorchRLAlgorithm2(
        trainer=trainer,
        ...
        sd_replay_buffer=sd_replay_buffer,
        goal_path_buffer=goal_path_buffer,
