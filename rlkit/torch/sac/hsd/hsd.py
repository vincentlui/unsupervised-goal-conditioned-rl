...
class HSDSchedulerTrainer(TorchTrainer):
    def __init__(
            self,
            env,
            scheduler,
    ...
    def train_skill_dynamics(self, batch):
        ...
        dyn_loss = -torch.mean(log_likelihood)
    ...
    def train_from_torch(self, batch):
        ...
        new_obs_actions, policy_mean, policy_log_std, log_pi, *_ = self.scheduler(
            start_states, skill_vec=start_skills, reparameterize=True, return_log_prob=True,
        )
        ...
        QF Loss
        """
        q1_pred = self.qf1(obs_skills, start_actions)
        q2_pred = self.qf2(obs_skills, start_actions)
        # Make sure policy accounts for squashing functions like tanh correctly!
        new_next_actions, _, _, new_log_pi, *_ = self.scheduler(
            end_states, skill_vec=start_skills, reparameterize=True, return_log_prob=True,
        )
